...
class LineFollowingNode:
    def __init__(self, name):
        ...
        self.result_publisher = rospy.Publisher(self.name + '/image_result', Image, queue_size=1)  # 图像处理结果发布(publish the image processing result)
        ...
        self.set_running_srv = rospy.Service(self.name + '/set_running', SetBool, self.set_running_srv_callback)  # 开启玩法(start the game)
        self.set_target_color_srv = rospy.Service(self.name + '/set_target_color', SetPoint, self.set_target_color_srv_callback)  # 设置颜色(set the color)
        self.get_target_color_srv = rospy.Service(self.name + '/get_target_color', Trigger, self.get_target_color_srv_callback)   # 获取颜色(get the color)
        self.set_threshold_srv = rospy.Service(self.name + '/set_threshold', SetFloat64, self.set_threshold_srv_callback)  # 设置阈值(set the threshold)
    ...
    def enter_srv_callback(self, _):
        ...
        with self.lock:
            ...
            set_servos(self.joints_pub, 1000, ((5, 500), ))  # 舵机中位(center the servo)
    ...
    def lidar_callback(self, lidar_data):
        ...
        if left_nonzero != np.array([]) and right_nonzero != np.array([]):
